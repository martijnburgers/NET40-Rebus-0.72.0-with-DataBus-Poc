...
namespace Rebus.KOOP.Autofac
{
    public static class AutofacConstants
    {
        public const string UnitOfWorkLifetime = "UnitOfWorkLifetime";
        public const string ContextKey = "AutofacLifetimeScope";
    }
    ...
    public class AutofacContainerAdapter : IContainerAdapter
    {
        ...
        public IEnumerable<IHandleMessages<T>> GetHandlerInstancesFor<T>()
        {
            ...
            var lifetimeScope = (ILifetimeScope)context.Items[AutofacConstants.ContextKey];

            IEnumerable<IHandleMessages<T>> handlers = lifetimeScope.Resolve<IEnumerable<IHandleMessages<T>>>();
        ...
        public void SaveBusInstances(IBus bus)
        {
            ...
            var uowManager = container.ResolveOptional<IUnitOfWorkManager>() ?? new AutofacUnitOfWorkManager(container);

            bus.Advanced.Events.AddUnitOfWorkManager(uowManager);
        }
        ...
        class AutofacUnitOfWorkManager : IUnitOfWorkManager
        {
            ...
            {
                var context = MessageContext.GetCurrent();
                var lifetimeScope = container.BeginLifetimeScope(AutofacConstants.UnitOfWorkLifetime);
                context.Items.Add(AutofacConstants.ContextKey, lifetimeScope);

                return new DefaultAutofacUnitOfWork(lifetimeScope);
            }
        }        
    }

    public class DefaultAutofacUnitOfWork : IUnitOfWork
    {
        readonly ILifetimeScope lifetimeScope;

        public DefaultAutofacUnitOfWork(ILifetimeScope lifetimeScope)
        {
